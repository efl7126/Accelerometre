...
void afficherGUI(float[] donnee) {
  ...
  fill(0, 102, 153);
  text("Numéro d'utilisateur : ", indent, 210);
  text(userID, indent + 160, 210);    
...
void envoiTrameSQL(float[] trame, int longueurTrame) {
  ...
  for(int i = 0; i < longueurTrame; i++)
  {
    ...
    if((indiceDonnee == 7) && (donnee[0] == 9999) && (donnee[4] == 9999))
    {
        ...
        if ( msql.connect() )
        {
            String horo = horodatage();
            
            println(horo);
          
            msql.query( "INSERT INTO `testPresentation` VALUES (NULL, %s,%s, %s, %s,%s,%s,%s,%s)", 
                     userID, horo, donnee[1], donnee[2], donnee[3], donnee[5], donnee[6], donnee[7]);
...
void setup() {
  ...
  {    
    ...
    String ip       = "192.168.0.164";  // the remote IP address
    ...
    String temps = horodatage();
    println("Initialisation du programme");
    println(temps);
    println("Veuillez entrer un numero d'utilisateur, suivi de Enter : ");
...
void keyPressed() {
        ...
        // Enregistrement d'un numéro d'utilisateur
        ...
        if (key == ENTER) {
          userID = mot;
          mot = "";  
          println("Numéro d'utilisateur sauvegardé : ");
          print("utilisateur "); 
          print(userID);
...
void receive( byte[] data, String ip, int port ) {	// <-- extended handler
  ...
  // Envoi de la trame sur la base de données
